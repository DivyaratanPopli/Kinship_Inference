...
def plot_betas(dfile, tfile, pair, thresh, rel, libraries, listf, Afile, pfile, pairf1, pairf2, p1thresh, instates, B1file, B2file, xnew1file, xnew2file, res1file, res2file, betaplot):
    ...
    for i in [0,1]:
        ...
        a=betas[2]
        b=betas[3]
        ax[i].plot(x, beta.pdf(x, a, b)/533.33,'#377eb8', lw=3, alpha=0.6, label="$Z_w$=0")
        #plt.hist(diff/total,alpha=0.3)

        a=betas[0]
        b=betas[1]
        ax[i].plot(x, beta.pdf(x, a, b)/533.33,'#ff7f00', lw=3, alpha=0.6, label="$Z_w$=1")

        a=betas[4]
        b=betas[5]
        ax[i].plot(x, beta.pdf(x, a, b)/533.33,'#4daf4a', lw=3, alpha=0.6, label="$Z_w$=2")
        #plt.hist(diff/total,alpha=0.3)

        a=betas[6]
        b=betas[7]

        ax[i].plot(x, beta.pdf(x, a, b)/533.33,'#f781bf', lw=3, alpha=0.6, label="$Z_w$=4")

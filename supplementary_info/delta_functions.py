...
def hmm(difffile, targets, totalfile, listind, listf, pfile, Afile, deltafile, upA,pairf1, pairf2, instates, p1thresh, thresh):
...
#    pair2.loc[pair2['g_noin']>0.5,'g_noin']=1
    ...
    gamma,A,B,up_p,lik, pi, xfinal= baum_welch(data=data, hbd=hbd, A=A, B=B, pos=pos, p1avg=initial_p, update_transition=upA, inbr=inbr_states, x0=x0)
    res=viterbi(data, A, B, pi)

    np.savetxt(fname=deltafile, X=xfinal,delimiter=',')

...
def baum_welch(data, A, B, pos, p1avg, log_bscale, x0, highdiff, max_iter=1000):
    ...
    while diff > 0.000001 and n_iter<max_iter:
        ...
        A, break1=transition(alpha1=alpha, beta1=beta, n1=scale, gamma1=gamma, A=A, B1=B, pos1=pos)
        if break1==1:
            break;
...
def hmm(dfile, tfile, ind, chrmf, p1file, resfile, likfile, upA):
    ...
    if win>40:              #if number of windows with nonzero count
        gamma,A,B,lik,break1= baum_welch(data=data, A=A, B=B, pos=pos, p1avg=initial_p, log_bscale=log_bscale, x0=x0, highdiff=highdiff)
    else:

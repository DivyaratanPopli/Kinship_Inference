...
KIn is able to differentiate between parent-child and siblings in many cases where READ can identify the pairs as first degree but there is not enough data for lcMLkin to give a result (suppl table2). Interestingly, we also find cases where READ predicts first degree, but KIn and lcMLKin predict differ in prediction of siblings and parent-child (supple table 3). Four such pairs have good amount of data, and a visual inspection shows that in all these cases, KIn seems to have the correct estimate (supp fig. ).

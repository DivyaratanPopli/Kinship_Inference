...
We further applied KIn to a genome-wide dataset of 118 ancient individuals sequenced and analyzed by Mittnik et al.,2019. We compared our relatedness estimates to those obtained from the authors using READ and lcMLkin, and found that in most cases the three methods agree. Out of 6903 pairs, 344 pairs show different relatedness for KIn and READ (suppl table1). We find only three pairs in this subset where lcMLkin has sufficient data, and for two of these pairs, KIn and lcMLKin predict relatedness as second degree, and READ predicts unrelated individuals. This is concurrent with the results from simulations where we find that KIn has higher power to detect second degree relatedness, compared to READ. We find only one pair in the entire dataset for which all three methods differ. For pair (POST131_POST28), READ predicts unrelated, KIn predicts second degree, while lcMLkin shows 3rd-5th degree relatedness.  
KIn is able to differentiate between parent-child and siblings in many cases where READ can identify the pairs as first degree but there is not enough data for lcMLkin to give a result (suppl table2). In many of these cases, KIn can differentiate between parent-child and siblings. Interestingly, we also find cases where READ predicts first degree, but KIn and lcMLKin predict differ in prediction of siblings and parent-child (supple table 3). Four such pairs have good amount of data, and a visual inspection shows that in all these cases, KIn seems to have the correct estimate (supp fig. ).

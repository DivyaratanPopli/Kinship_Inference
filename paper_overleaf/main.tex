...
We further applied KIN to a genome-wide dataset of 118 ancient individuals from the Lech Valley \cite{mittnik_kinship-based_2019}. We compared our relatedness estimates to those obtained by the authors using READ and lcMLkin. We found that in most cases the three methods agree. Only $<5\%$ of pairs (315 out of 6903) show a different relatedness for KIN and READ. However, most of these are at very low confidence for both the methods ($\Delta LL<1$ for KIN and $sd<1$ for READ). There are 28 pairs where KIN shows confident results (table S2), and when we plot the raw heterozygosity estimates along the genome, in many cases it is possible to see that the prediction by KIN is in agreement with the observed proportion of differences along the genome (Fig. S11). Comparisons with lcMLkin are sparse since lcMLkin shows reliable results for $\appr 53\%$ pairs (\cite{mittnik_kinship-based_2019}), and we found 16 pairs for which lcMLkin gives a reliable result, and KIN shows $\Delta LL>$1 (table S3). We again investigated these comparisons in more detail, and found that at least in cases where there is good amount of overlapping sites ($\geq35,000$), we can clearly see that the proportion of differences along the genome aligns with the prediction of KIN (Fig. S12). 
...
Conceptualization (Design of study): B.M.P.; Software: D.P.; Methodology—lead: B.M.P.; Methodology—support: D.P.; Formal Analysis: D.P.; Visualization-lead: D.P.; Visualization-support: S.P.; Data Curation: D.P.; Writing (original draft preparation)—lead: D.P.; Writing (original draft preparation)—support: B.M.P.; Writing (review and editing): input from all authors; Supervision: B.M.P.

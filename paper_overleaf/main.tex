...
\begin{figure}[h]
    \centering
    \includegraphics[width=18cm]{plots/inkscape_finalImg/schematic.png}
    \caption{}
    \label{fig1:ibd}
\end{figure}
...
P(D_{w}|p_j,\delta_j,N_w) = \binom{N_w}{D_w}\frac{B(D_w+p_j \delta_{j}, N_w-D_w+ \delta_{j}(1-p_{j}))}{ B(p_{j}\delta_{j}, (1-p_{j})\delta_{j})}
\end{equation}
...
The only free parameters we estimate in the M-step are the overdispersion parameters $\delta_j$. We do this optimization using a cost function, which is the log-emission probability weighted by the posterior probabilities of the hidden states $\gamma_{wi}$ and the ROH state-probabilities and $h_{w\omega}$.

...
make_panel1 <- function(data,acrval,leg){
  plot1<-ggplot(data=data[data$acr==acrval,], aes(x=Wins, y=Prop)) +
    ...
    theme_void()
  plot1<-plot1+geom_line(aes(colour=Coverage),alpha=0.3, size=0.8)
  plot1<-plot1 + theme_bw()+ theme(legend.position = "none", axis.title.x = element_blank())+labs(y= "Heterozygosity") +
...
make_panel2 <- function(data,acrval,leg){
  ...
  #plot2
  plot2<-plot2+geom_line(aes(x=Wins, y=Model_pred, linetype="Inferred", colour=Coverage), alpha=0.3, size=0.8)
  ...
  if (leg==1){
    ...
    plot2<-plot2 + theme_bw() + theme(legend.position = "none", axis.title.x = element_blank())+labs(y= "1 - P(ROH)")
  }
  plot2<-plot2 +
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
...
ROH_plot <- function(infile,outfile){
  ...
  panel21 <- make_panel1(data=data, acrval='0_0_1',leg=0)
  panel12 <- make_panel2(data=data, acrval='0_0_0',leg=0)
  panel22 <- make_panel2(data=data, acrval='0_0_1',leg=0)
  
  panel31 <- make_panel1(data=data, acrval='2_0_1',leg=0)
  panel41 <- make_panel1(data=data, acrval='0_1_1',leg=0)
  panel32 <- make_panel2(data=data, acrval='2_0_1',leg=0)
  panel42 <- make_panel2(data=data, acrval='0_1_1',leg=0)
  
  panel<-plot_grid(panel11,panel12,panel21,panel22,panel31,panel32,panel41,panel42, labels= c("A"," ","B"," ","C"," ","D"," "), ncol=1, rel_widths = c(1,1,1,1,1,1,1,1), rel_heights =  c(1,1,1,1,1,1,1,1), align="v",axis="lr", label_size=10, label_x=0, label_y=1, hjust = 0, vjust = 3)
  panel
...
#infile='/mnt/diversity/divyaratan_popli/100arc/inbreeding/model_git/plots/plotdata/plot_ROH.csv'
